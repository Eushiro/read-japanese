...
**All AI model names are centralized in `web/convex/lib/models.ts`** - the single source of truth.

**Model Constants:**

```typescript
import { TEXT_MODELS, AUDIO_MODELS, IMAGE_MODELS } from "./lib/models";

// Use constants, never hardcode model names
const result = await generateText({
  model: TEXT_MODELS.GEMINI_3_FLASH, // ✅ Correct
  // model: "gemini-3-flash",          // ❌ Wrong - ESLint will block
  prompt: "...",
});
```
...
- `TEXT_MODELS` - Text generation models (Gemini, Claude, GPT)
- `AUDIO_MODELS` - Text-to-speech models (Gemini TTS, ElevenLabs)
- `IMAGE_MODELS` - Image generation models (DALL-E, Imagen)

**Provider Routing:**
The system automatically routes to the correct provider:

- **Google Direct API** - For Gemini models (lower latency, native features)
- **OpenRouter** - For Claude, GPT, and other models (unified API)

**Model Chains (Fallback):**

```typescript
import { MODEL_CHAINS } from "./lib/models";

// If primary model fails, falls back to alternatives
// MODEL_CHAINS.ASSESSMENT = [GEMINI_3_FLASH, CLAUDE_SONNET, GPT_4O]
```

**ESLint Enforcement:**
An ESLint rule in `web/eslint.config.js` prevents hardcoded model names:

```javascript
// This will be flagged as an error in convex/ files:
const model = "gemini-3-flash"; // ❌ ESLint error

// Use the constant instead:
const model = TEXT_MODELS.GEMINI_3_FLASH; // ✅ OK
```

**Adding New Models:**

1. Add to `web/convex/lib/models.ts` with proper typing
2. Add provider routing logic if using a new provider
3. Update model chains if applicable for fallback behavior

**Generation Functions:**
Use the functions from `web/convex/ai/models.ts` for text generation:

```typescript
import { generateText, generateAndParse } from "./ai/models";
...
const result = await generateAndParse<MyType>({
  model: TEXT_MODELS.GEMINI_3_FLASH,
  systemPrompt: "...",
  userPrompt: "...",
  schema: myZodSchema, // Optional: validates output
});
```

**Provider Files:**

- `web/convex/ai/providers/google.ts` - Direct Google API for Gemini models
- `web/convex/ai/providers/openrouter.ts` - OpenRouter API for Claude, GPT, etc.
- `web/convex/ai/providers/index.ts` - Routes to the correct provider automatically
