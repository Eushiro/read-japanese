  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-2xl">
        ...
        {/* Deck List */}
        <div className="overflow-y-auto space-y-2 pr-2 h-[50vh]">
          {displayDecks === undefined ? (
            ...
            // No decks available - centered empty state
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1.5">
                      <span
                        className={`text-base font-medium ${selectedDeckId === deck.deckId ? "text-accent" : "text-foreground"}`}
                      >
                        {deck.name}
                      </span>
                      {getLevelVariant(deck.level) ? (
                        ...
                        <span className="text-xs px-2 py-0.5 rounded bg-muted text-foreground-muted">
                          {deck.level}
                        </span>
                      )}
                    </div>
                    <p className="text-sm text-foreground-muted line-clamp-2">{deck.description}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <div className="text-lg font-semibold text-foreground">{deck.totalWords}</div>
                    <div className="text-sm text-foreground-muted">{t("vocabulary.words")}</div>
                  </div>
                </div>
